...
export const TEST_PROJECT = join(process.cwd(), 'test-near-willem-workspaces-init');
...
for (const file of [
  'near-willem-workspaces/.gitignore',
  'near-willem-workspaces/README.md',
  'near-willem-workspaces/__tests__/main.ava.ts',
  'near-willem-workspaces/ava.config.cjs',
  'near-willem-workspaces/package.json',
  'near-willem-workspaces/tsconfig.json',
  'test.bat',
  'test.sh',
]) {
...
test('using --no-install skips installing node_modules', async t => {
  t.assert(!await pathExists(`${TEST_PROJECT}/near-willem-workspaces/node_modules`));
});

test('package.json includes correct version of near-willem-workspaces-ava', t => {
  ...
  const packageJson = require(join(TEST_PROJECT, 'near-willem-workspaces/package.json')); // eslint-disable-line @typescript-eslint/no-var-requires, @typescript-eslint/no-require-imports
  t.is(packageJson.devDependencies!['near-willem-workspaces-ava'], version); // eslint-disable-line @typescript-eslint/no-unsafe-member-access
});

test('package.json is well-formatted with line breaks', async t => {
  t.regex(
    (await fs.readFile(join(TEST_PROJECT, 'near-willem-workspaces/package.json'))).toString(),
    /{\n/,
...
test('tests pass in new project since it is nested in monorepo and has access to monorepo dependencies', t => {
  const testRun = spawnSync('yarn', [
    ...
    cwd: join(TEST_PROJECT, 'near-willem-workspaces'),
  });
